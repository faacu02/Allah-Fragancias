    ...
    <div className="md:hidden grid grid-cols-1 gap-4">
      {products.map((product) => (
        <div key={product.id} className="bg-darker border border-gold/15 p-4 flex gap-4 items-start">
          <div className="w-16 h-20 bg-dark overflow-hidden flex-none cursor-pointer relative" onClick={() => onPreview(product)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onPreview(product); }} role="button" tabIndex={0}>
            <Image
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-3 text-xs">
              ...
              <span className={`text-[10px] px-1.5 py-0.5 uppercase font-bold ${product.status === 'LOW' ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'}`}>{product.status === 'LOW' ? 'Low Stock' : product.status === 'OK' ? 'OK' : product.status}</span>
            </div>
